...
Cover image for How to Dockerize a Next.js application
vorillaz
vorillaz
Posted on Jan 17 • Updated on Jan 25
...
Anton Prudkohliad
•
Jul 18 • Edited on Jul 18

Hi!
Thanks for the effort you put into this post, it should help people get started quickly.
However, while I was reading it and the original Next.js guide for deploying with Docker, I noticed that the .env.production file is copied into the Docker image. Would not that result in a secrets exposure? Isn't anything copied into the image potentially extractable? Especially considering that in the end of the post it's suggested to publish the image to DockerHub.
...
Can You Trust Your Login Security?
2FA, MFA and OTP solutions are the barrier between secure logins and fraudsters - but are the effective? Take your login security to the next level with Fingerprint for ATO prevention.
